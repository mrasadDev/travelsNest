...
export default function PhotosUploader({ addedPhotos, onChange }) {
  ...
  function uploadPhoto(ev) {
    ...
    axios
      ...
      .then((response) => {
        ...
        console.log(uploadedFileNames[0], ">>>>>>>>>>>>>>>.");
        // Assuming the server returns an array of filenames
        setUploadedImages((prev) => {
          if (prev.includes(uploadedFileNames[0])) {
            console.log(">>>>>>>>>");
            return prev;
          }
        ...
        //Preload the image to make it appear in the network tab
        const img = new Image();
        img.src = `/uploads/${uploadedFileNames[0]}`;
  ...
  const allPhotos = [...new Set([...addedPhotos, ...uploadedImages])];
  ...
  return (
    <>
      ...
      <div className="mt-2 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
        {allPhotos.length > 0 &&
          allPhotos.map((link, index) => (
            <div className="h-32 flex relative" key={`photo_${index}`}>
              <Image
